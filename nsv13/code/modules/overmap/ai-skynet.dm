...
/datum/fleet
	var/name = "Cylon Invasion Fleet"//Todo: randomize this name
	...
	var/faction_id = FACTION_ID_CYLON
...
/datum/fleet/proc/move(datum/star_system/target, force=FALSE)
	...
	if(!hide_movements && !current_system.hidden)
		(alignment != "kobol") && mini_announce("Typhoon drive signatures detected in [current_system]", "White Rapids EAS")
...
/datum/fleet/proc/defeat()
	...
	if(alignment == "kobol" || current_system == player_system || current_system == mining_system)
...
/datum/fleet/kobol/earth/defeat()
...
/datum/fleet/neutral
	name = "\improper Cylon Scout Fleet 6-1"
	fleet_trait = FLEET_TRAIT_NEUTRAL_ZONE

/datum/fleet/border
	name = "\improper Cylon Border Enforcement Fleet"
...
/datum/fleet/boarding
	name = "\improper Cylon Legionary Fleet"
	destroyer_types = list(/obj/structure/overmap/syndicate/ai/assault_cruiser/boarding_frigate)
	taunts = list("Attention: This is an automated message. All non-Cylon vessels prepare to meet their maker.")
	fleet_trait = FLEET_TRAIT_NEUTRAL_ZONE

/datum/fleet/wolfpack
	name = "\improper unidentified fleet"
...
/datum/fleet/conflagration
	name = "\improper Cylon Conflagaration Fleet"
	taunts = list("Enemy ship, surrender now. This vessel is armed with prototype weapons and is eager to test them.")
	audio_cues = list()
...
/datum/fleet/elite
	name = "\improper elite Cylon taskforce"
...
/datum/fleet/pirate/tortuga
	...
	audio_cues = list()
	taunts = list("These are our waters you are sailing, prepare to surrender!", "Bold of you to fly Colonial colours in this system, your last mistake.")
...
/datum/fleet/rubicon //Crossing the rubicon, are we?
	...
	audio_cues = list()
	taunts = list("Better crews have tried to cross the Rubicon, you will die like they did.", "Defense force, stand ready!", "Colonial filth. Munitions, ready the guns. We’ll scrub the galaxy clean of you vermin.", "This shift just gets better and better. I’ll have your Captain’s head on my wall.")
	fleet_trait = FLEET_TRAIT_DEFENSE

/datum/fleet/earthbuster
	name = "\proper Cylon Armada" //Fleet spawned if the players are too inactive. Set course...FOR EARTH.
	...
	size = FLEET_DIFFICULTY_VERY_HARD
	allow_difficulty_scaling = FALSE
	taunts = list("We're coming for the colonies, and you can't stop us. All batteries fire at will.", "Lay down your arms now, you're outnumbered.", "All hands, assume assault formation. Begin bombardment.")
	audio_cues = list()

/datum/fleet/interdiction	//Pretty strong fleet with unerring hunting senses, Adminspawn for now.
	name = "\improper Cylon Interdiction Fleet Gamma-1"	//These fun guys can and will hunt the player ship down, no matter how far away they are.
...
/datum/fleet/interdiction/stealth	//More fun for badmins
	name = "\improper unidentified heavy fleet"
	hide_movements = TRUE
...
/datum/fleet/interdiction/light	//The syndicate can spawn these randomly (though rare). Be caareful! But, at least they aren't that scary.
	name = "\improper Cylon Light Interdiction Fleet Sci-6"
...
/datum/fleet/unknown_ship
	name = "\improper unknown Cylon ship class"
	size = 1
	allow_difficulty_scaling = FALSE
...
/datum/fleet/kobol
	name = "\improper Colonial Line Fleet"
	...
	faction_id = FACTION_ID_KOBOL
	taunts = list("Cylon vessel, stand down or be destroyed", "You are encroaching on our airspace, prepare to be destroyed", "Unidentified vessel, your existence will be forfeit in accordance with the peacekeeper act.")

/datum/fleet/kobol/light
	name = "\improper Colonial S-Fleet"
	battleship_types = list(/obj/structure/overmap/nanotrasen/patrol_cruiser/ai)

/datum/fleet/kobol/border
	name = "\proper Colonial Border Patrol Detachment"
...
/datum/fleet/kobol/border/defense
	name = "\proper 4th Fleet, 23rd Detachment 'Olympus' Fringe Battlegroup"
	taunts = list("You have violated the law. Stand down your weapons and prepare to be boarded.", "Hostile vessel. Stand down immediately or be destroyed.")
	size = FLEET_DIFFICULTY_EASY
	fleet_trait = FLEET_TRAIT_DEFENSE
...
/datum/fleet/solgov
	name = "\improper Colonial Light Pathfinding Fleet"
...
/datum/fleet/solgov/earth
	name = "\proper Caprica Defense Detachment"
...
/datum/fleet/solgov/interdiction
	name = "\improper Colonial Interdiction Fleet"
...
/datum/fleet/proc/federation_check(checked = current_system) //Lazy way to check if you're in the federation; for alignments.
	if(istype(checked, /datum/star_system))
		...
		if(S.owner == "solgov" && alignment == "kobol")
			return TRUE
		if(S.owner == "kobol" && alignment == "solgov")
			return TRUE
	if(istype(checked, /datum/fleet))
		var/datum/fleet/F = checked
		if(F.alignment == "solgov" && alignment == "kobol")
			return TRUE
		if(F.alignment == "kobol" && alignment == "solgov")
			return TRUE
	if(istype(checked, /obj/structure/overmap))
		var/obj/structure/overmap/O = checked
		if(O.faction == "solgov" && alignment == "kobol")
			return TRUE
		if(O.faction == "kobol" && alignment == "solgov")
			return TRUE
	return FALSE
...
/datum/fleet/proc/add_ship(var/obj/structure/overmap/member, role as text)
	...
	if(alignment != "kobol" && alignment != "solgov") //NT, SGC or whatever don't count as enemies that NT hire you to kill.
...
/datum/starsystem_manager/ui_data(mob/user)
	...
	for(var/datum/star_system/SS in SSstar_system.systems)
		...
		for(var/datum/fleet/F in SS.fleets)
			...
			fleet_info["id"] = "\ref[F]"
			if(F.alignment == "kobol" || F.alignment == "solgov")
				fleet_info["color"] = "good"
			else if(F.alignment == "cylon" || F.alignment == "pirate")
		...
		for(var/obj/object in (SS.system_contents))
			...
			if(istype(object, /obj/structure/overmap))
				...
				if(!OM.fleet)
					...
					if(OM.faction == "kobol" || OM.faction == "solgov")
						overmap_info["color"] = "good"
					else if(OM.faction == "cylon" || OM.faction == "pirate")
