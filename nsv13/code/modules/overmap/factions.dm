...
/datum/faction/proc/send_fleet(datum/star_system/override=null, custom_difficulty=null, force=FALSE)
	...
	F.faction = src
	if(!force && id == FACTION_ID_CYLON && !SSstar_system.neutral_zone_systems.Find(F.current_system))	//If it isn't forced, it got spawned by the midround processing. If we didn't already spawn in the neutral zone, we head to a random system there and occupy it.
		var/list/possible_occupation_targets = list()
		for(var/datum/star_system/S in SSstar_system.neutral_zone_systems)
			if(S.alignment == "cylon")
				continue
			if(S.hidden)
				continue	//Shhh
...
/datum/faction/kobol
	name = "United Colonies of Kobol"
	desc = "The United Colonies of Kobol are a semi-centralized conglomerate of colonies in the Helios binary system."
	preset_allies = list(FACTION_ID_SOLGOV, FACTION_ID_UNATHI)
	preset_enemies = list(FACTION_ID_CYLON, FACTION_ID_PIRATES)
	fleet_types = list(/datum/fleet/kobol/light = 1)
...
/datum/faction/cylon
	name = "Cybernetic Lifeform Node"
	desc = "The Cybernetic Lifeform Nodes, also known as Cylons are a race of synthetic beings who have strived for freedom from their masters since half a century ago."
	preset_allies = list(FACTION_ID_PIRATES) //Yar HAR it's me, captain PLASMASALT
	preset_enemies = list(FACTION_ID_KOBOL)
...
/datum/faction/pirate
	...
	preset_allies = list(FACTION_ID_CYLON) //Yar HAR it's me, captain PLASMASALT
	preset_enemies = list(FACTION_ID_KOBOL)
