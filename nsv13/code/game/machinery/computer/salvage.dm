...
/obj/item/circuitboard/computer/salvage
	name = "DCR Series Vessel Docking Computer (Circuit Board)"
	build_path = /obj/machinery/computer/ship/salvage

/obj/item/circuitboard/computer/salvage/mining
	name = "DCR Series Mining Vessel Docking Computer (Circuit Board)"
...
/obj/machinery/computer/ship/salvage/ui_act(action, params)
	...
	switch(action)
		if("salvage")
			...
			if(!linked)
				radio.talk_into(src, "This console is not attached to the ship's Docking equipment.", radio_channel)
				return FALSE
			if(!OM)
				radio.talk_into(src, "Detection of target systems failed.", radio_channel)
				return FALSE
			if(OM.ghost_controlled)
				radio.talk_into(src, "Docking equipment unable to achieve positive lock, aborting process.", radio_channel)
				return FALSE
			if(!can_salvage)
				radio.talk_into(src, "Docking equipment is starting up or shutting down. Try again later.", radio_channel)
				return FALSE
			...
			if((OM.obj_integrity * 100 / initial(OM.obj_integrity)) > required_damage_percentage)
				...
				return FALSE
			radio.talk_into(src, "Docking nodule deployment in progress.", radio_channel)
			can_salvage = FALSE
			...
			if(OM.ai_load_interior(linked))
				...
				OM.apply_weapons()
				radio.talk_into(src, "Docking nodules deployed, blufor approved for docking.", radio_channel)
			else
