/datum/overmap_objective/board_ship
	...
	brief = "Capture the Cylon vessel CALLANADMIN by boarding it, defeating the enemies therein, and modifying its IFF codes."
...
/datum/overmap_objective/board_ship/instance()
	...
	brief = "Capture the Cylon vessel [target_ship] in [target_system] by boarding it, defeating the enemies therein, and modifying its IFF codes."
	target_system.add_ship(target_ship)
	target_system.enemies_in_system += target_ship
	target_system.objective_sector = TRUE
	// give it a friend :)
	var/datum/faction/S = SSstar_system.faction_by_id(FACTION_ID_CYLON)
