...
/datum/antagonist/nukeop/syndi_crew/greet()
	...
	to_chat(owner, "<span class='warning'>Ensure the destruction of [station_name()], no matter what. Eliminate Colonial presence in the Abassi ridge before they can establish a foothold. The fleet is counting on you!</span>")
...
/datum/syndicate_crew_role/captain
	...
	desc = "Wield the might of an advanced Syndicate battlecruiser and ensure Colonial presence in this sector is severely diminished. Primary duties: Lead the ship into battle, coordinate with the strategist and enact any maneuvers they see fit, outperform your Colonial counterpart."
...
/datum/antagonist/nukeop/leader/syndi_crew/greet()
	...
	to_chat(owner, "<span class='warning'>Diminish Colonial presence in this sector by destroying NT fleets and claiming systems with your lighthouse beacon. Destruction of the [station_name()] via superstructure crit or nuclear detonation are also options. For Abassi!</span>")
	...
	if(istype(SSticker.mode, /datum/game_mode/pvp))
		...
		if(result == NUKE_RESULT_NUKE_WIN)
			...
			switch(mode.winner.id)
				if(FACTION_ID_KOBOL)
					parts += "<span class='redtext big'>Colonial Major Victory!</span>"
