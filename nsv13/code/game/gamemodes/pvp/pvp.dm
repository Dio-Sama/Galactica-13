...
/datum/game_mode/pvp/post_setup()
	...
	var/datum/faction/synd = SSstar_system.faction_by_id(FACTION_ID_CYLON)
	var/datum/faction/nt = SSstar_system.faction_by_id(FACTION_ID_KOBOL)
...
/datum/game_mode/pvp/proc/force_loss()
	winner = SSstar_system.faction_by_id(FACTION_ID_KOBOL)
	check_finished()
	SSticker.force_ending = TRUE

//And this subsequently happens if the nsv is taken out
/datum/game_mode/pvp/proc/force_win()
	winner = SSstar_system.faction_by_id(FACTION_ID_CYLON)
	check_finished()
	SSticker.force_ending = TRUE

/datum/game_mode/pvp/check_win()
	if(winner)
		if(winner.id != FACTION_ID_KOBOL)
			return TRUE
		else
			return FALSE
	if(nukes_left == 0)
...
/datum/game_mode/pvp/set_round_result()
	...
	switch(winner.id)
		if(FACTION_ID_KOBOL)
			SSticker.mode_result = "loss - Colonials repelled the invasion"
		...
		if(FACTION_ID_CYLON)
			...
			return
	SSticker.mode_result = "syndicate minor loss! - Colonial allies were able to repel the invasion."
	SSticker.news_report = PVP_SYNDIE_LOSS
