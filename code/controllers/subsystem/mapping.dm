...
///K13 Stationmap fix attempt.
	if(!config.stationmap_disable)
		instance_overmap(_path=config.station_type, folder= config.station_path ,interior_map_files = config.station_file, traits=config.station_traits)
