...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, auth_id)
	...
	if(is_silicon)
		...
		priority_announce(html_decode(user.treat_message(input)), null, 'sound/misc/announce_dig.ogg', "Captain", has_important_message = TRUE, auth_id = auth_id)
		nonsilicon_message_cooldown = world.time + COMMUNICATION_COOLDOWN
	user.log_talk(input, LOG_SAY, tag="shipwide announcement")
	message_admins("[ADMIN_LOOKUPFLW(user)] has made a shipwide announcement.")
