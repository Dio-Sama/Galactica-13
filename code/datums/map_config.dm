...
/datum/map_config //NSV EDITED START
	...
	var/station_type = /obj/structure/overmap/trader/gantry //K13
	var/mine_disable = FALSE //NSV13 mining disable
	var/stationmap_disable = TRUE //K13 option- allows for disabling of various support space stations!
	var/station_file = "orbitalhabitat.dmm" //K13 option
	var/mine_file = "Rocinante.dmm" //Nsv13 option
	var/mine_path = "map_files/Mining/nsv13" //NSV13 option
	var/station_path = "templates/boarding" //K13 option
...
/datum/map_config/proc/LoadConfig(filename, error_if_missing)
	...
	if(!("stationmap_disable" in json)) //same as above, except with stationmap
		station_file = json["station_file"]
		station_path = json["station_path"]
		// "map_file": "BoxStation.dmm"
		if (istext(station_file))
			if (!fexists("_maps/[station_path]/[station_file]"))
				log_world("Map file ([station_path]/[station_file]) does not exist!")
				return
		// "map_file": ["Lower.dmm", "Upper.dmm"]
		else if (islist(station_file))
			for (var/file in station_file)
				if (!fexists("_maps/[station_path]/[file]"))
					log_world("Map file ([station_path]/[file]) does not exist!")
					return
		else
			log_world("station_file missing from json!")
			return

		CHECK_EXISTS("station_type")
		if("station_type" in json)
			station_type = text2path(json["mining_ship_type"])
		else
			log_world("station_type missing from json!")
			return
	...
	station_traits = json["station_traits"]
	// "traits": [{"Linkage": "Cross"}, {"Space Ruins": true}]
	if (islist(station_traits))
		// "Station" is set by default, but it's assumed if you're setting
		// traits you want to customize which level is cross-linked
		for (var/level in station_traits)
			if (!(ZTRAITS_BOARDABLE_SHIP in level))
				level += ZTRAITS_BOARDABLE_SHIP
	// "traits": null or absent -> default
	else if (!isnull(station_traits))
		log_world("station_traits is not a list!")
		return
